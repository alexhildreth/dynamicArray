...
*
Date: 11/20/14
** Description: Implimentation of dynamicArray.h. Creates and
** manages a dynamic array data structure. Allows for
** addition to, subtraction from, and search of the array.
** Adds additional support for heap tree functionality for use
** in toDoList.c.
**
**
** Input: n/a
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
   ...
   {
   		smaller = _smallerIndexHeap(heap, leftChild, rightChild);
   		
   		if(compare(getDynArr(heap,pos), getDynArr(heap,smaller)) == 1)
   		{
...
{
    int firstNonLeaf = (heap->size / 2) - 1;
    int max = heap->size;
    
    //decrement down through each non-leaf node and adjust
    while(firstNonLeaf > -1)
    {
    	_adjustHeap(heap, max, firstNonLeaf);
    	firstNonLeaf--;
    }
}
/*
